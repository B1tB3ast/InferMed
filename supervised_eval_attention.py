...
import torch
import numpy as np
import supervised_with_attention  # Import your training script
...
if __name__ == "__main__":
    ...
    val_loader, device = supervised_with_attention.load_data(embeddings_val_path)

    # Load the trained model
    model = supervised_with_attention.FusionModel().to(device)
    model.load_state_dict(torch.load("checkpoints_supervised_model_attention/model_epoch_60.pth", map_location=device,weights_only=True))  # Load last checkpoint
